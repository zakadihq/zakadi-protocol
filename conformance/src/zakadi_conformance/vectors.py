...
def write_message_vectors(root: Path) -> None:
    ...
    for direction, valid_map, extra_map, invalid_map in (
        (
            "client",
            examples.VALID_CLIENT,
            examples.EXTRA_VALID_CLIENT,
            examples.INVALID_CLIENT,
        ),
        (
            "server",
            examples.VALID_SERVER,
            examples.EXTRA_VALID_SERVER,
            examples.INVALID_SERVER,
        ),
    ):
...
def framing_cases() -> list[dict]:
    ...
    def valid(
        name: str, description: str, header: Header, payload: bytes, **extra
    ) -> None:
    ...
    valid(
        "video-idr-with-params",
        "Video IDR at rung 2 with SPS and PPS preceding the slice (Annex-B start codes).",
        Header(type=0, seq=1042, pts_ms=8420, rung=2, keyframe=True, param_sets=True),
        _annexb([7, 8, 5], 1),
    )
    valid(
        "video-p-frame",
        "Video non-IDR slice, no parameter sets.",
        Header(type=0, seq=1043, pts_ms=8487, rung=2),
        _annexb([1], 2),
    )
    valid(
        "video-rung-changed",
        "First video message after a rung change to rung 3; a rung JSON message must accompany it.",
        Header(
            type=0,
            seq=1044,
            pts_ms=8554,
    ...
    valid(
        "audio-opus-packet",
        "One 20 ms Opus packet at rung 2.",
        Header(type=1, seq=511, pts_ms=8420, rung=2),
        _pattern(61, 4),
    )
    batch = [(0, _pattern(58, 5)), (20, _pattern(60, 6)), (40, _pattern(57, 7))]
    valid(
        "audio-batch-three-records",
        "Three Opus packets batched at rung 4; the batch consumes one audio seq value.",
        Header(type=3, seq=512, pts_ms=8440, rung=4),
        build_audio_batch(batch),
        audio_batch=[{"pts_delta_ms": d, "packet_hex": p.hex()} for d, p in batch],
    )
    probe_payload = struct.pack("<Q", 1200450) + _pattern(48, 8)
    valid(
        "probe",
        "Probe message: uint64 little-endian send time in microseconds, then filler; probes have their own seq space.",
        Header(type=2, seq=0, pts_ms=0, rung=0),
        probe_payload,
        probe_send_time_us=1200450,
    )
    valid(
        "seq-max",
        "seq at its maximum value; the next message of this type carries seq 0.",
        Header(type=0, seq=65535, pts_ms=60000, rung=2),
        _annexb([1], 9),
    )
    valid(
        "seq-wrapped",
        "The message following seq 65535 carries seq 0 (wraparound is not a gap).",
        Header(type=0, seq=0, pts_ms=60067, rung=2),
        _annexb([1], 10),
    )
    valid(
        "pts-max",
        "pts_ms at its maximum uint32 value.",
        Header(type=1, seq=7, pts_ms=4294967295, rung=1),
        _pattern(40, 11),
    )
    valid(
        "all-flags-rung-15",
        "Every flag set and rung 15 (the header allows rungs the ladder does not use).",
        Header(
            type=0,
            seq=1,
            pts_ms=1,
    ...
    good = encode_message(
        Header(type=0, seq=1, pts_ms=1, rung=2, keyframe=True), _annexb([5], 13)
    )
    invalid(
        "invalid-version",
        "Framing version bits set to 1.",
        bytes([good[0] | 0x40]) + good[1:],
        "unsupported_version",
    )
    invalid(
        "invalid-reserved-bit0",
        "Byte 0 bit 0 set.",
        bytes([good[0] | 0x01]) + good[1:],
        "reserved_bit_set",
    )
    invalid(
        "invalid-reserved-byte1",
        "Byte 1 low nibble set.",
        good[:1] + bytes([good[1] | 0x05]) + good[2:],
        "reserved_bits_set",
    )
    invalid("short-header", "Only five bytes.", good[:5], "short_header")
    invalid(
        "short-probe",
        "Probe payload shorter than the 8-byte send time.",
        encode_message(Header(type=2, seq=1, pts_ms=0), b"\x01\x02\x03\x04"),
        "short_probe",
    )
    invalid(
        "truncated-batch",
        "Batch record declares 100 bytes but carries 3.",
        encode_message(
            Header(type=3, seq=2, pts_ms=100, rung=4),
            struct.pack("<HH", 100, 0) + b"\x01\x02\x03",
        ),
        "truncated_batch_record",
    )
    invalid(
        "batch-out-of-order",
        "Batch records with decreasing pts_delta_ms.",
        encode_message(
            Header(type=3, seq=3, pts_ms=100, rung=4),
            build_audio_batch([(20, b"\x01"), (0, b"\x02")]),
        ),
        "batch_out_of_order",
    )
    invalid(
        "empty-batch",
        "Audio batch with no records.",
        encode_message(Header(type=3, seq=4, pts_ms=100, rung=4), b""),
        "empty_batch",
    )
    return cases
...
def chain_cases() -> list[dict]:
    ...
    for name, description, messages in (
        (
            "basic",
            "Five media messages including one probe (excluded) and one audio batch (included).",
            [
                encode_message(
                    Header(
                        type=0, seq=0, pts_ms=0, rung=2, keyframe=True, param_sets=True
                    ),
                    _annexb([7, 8, 5], 21),
                ),
                encode_message(
                    Header(type=1, seq=0, pts_ms=0, rung=2), _pattern(61, 22)
                ),
                encode_message(
                    Header(type=2, seq=0, pts_ms=0, rung=0),
                    struct.pack("<Q", 5000) + _pattern(56, 23),
                ),
                encode_message(
                    Header(type=3, seq=1, pts_ms=20, rung=2),
                    build_audio_batch([(0, _pattern(60, 24)), (20, _pattern(59, 25))]),
                ),
                encode_message(
                    Header(type=0, seq=1, pts_ms=67, rung=2), _annexb([1], 26)
                ),
            ],
        ),
        ...
        (
            "probes-only",
            "Only probes were sent: the chain still equals H0.",
            [
                encode_message(
                    Header(type=2, seq=i, pts_ms=0, rung=0),
                    struct.pack("<Q", 1000 + i) + _pattern(56, 30 + i),
                )
                for i in range(3)
            ],
        ),
        ...
        for m in messages:
            ...
            entries.append(
                {"hex": m.hex(), "chained": chained, "chain_after": chain.hex}
            )
        cases.append(
            {
                "name": name,
                "description": description,
                "session_id": examples.SESSION_ID,
                "jti": examples.JTI,
                "token": examples.TOKEN,
                "h0": h0(examples.SESSION_ID, jti).hex(),
                "messages": entries,
                "attest": {
                    "t": "attest",
                    "video_seq": vseq,
...
def write_transcripts(root: Path) -> None:
    ...
    for name, builder in examples.TRANSCRIPTS.items():
        ...
        (d / (name + ".jsonl")).write_text(
            "".join(json.dumps(line, sort_keys=False) + "\n" for line in lines),
            encoding="ascii",
        )
...
They belong to one session, `%(session_id)s` (`ses_` and a ULID), with one jti and one attest_nonce,
carried by one client token.

- `keys/jwks.json` is the public JWKS of the test key that signs the vector tokens, kid `%(kid)s`.
...
def generate(root: Path) -> None:
    ...
    (root / "vectors" / "streams" / "README.md").write_text(
        STREAMS_README, encoding="ascii"
    )
    (root / "vectors" / "README.md").write_text(
        VECTORS_README
        % {"session_id": examples.SESSION_ID, "kid": jws.load_private_jwk()["kid"]},
        encoding="ascii",
    )
